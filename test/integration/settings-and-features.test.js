...
test('admin users API hides AI bot-backed users and rejects admin actions for them', async () => {
  ...
  try {
    ...
    const botToken = await admin.request(`/api/admin/users/${botUserId}/tokens`, {
      method: 'POST',
      expectedStatus: 400,
    });
    assert.equal(botToken.data.error, 'AI bots are managed from the AI bot settings');
  } finally {
    db.close();
  }
});

test('personal API tokens can be generated by users and admins', async () => {
  const { admin, bob } = scenario;

  const finite = await bob.request('/api/auth/tokens', {
    method: 'POST',
    json: { expiresInSeconds: 3600 },
  });
  assert.equal(finite.data.token_type, 'Bearer');
  assert.equal(finite.data.expires_in_seconds, 3600);
  assert.equal(finite.data.never_expires, false);
  assert.ok(finite.data.expires_at);
  const finiteDecoded = jwt.decode(finite.data.token);
  assert.equal(finiteDecoded.id, bob.user.id);
  assert.ok(finiteDecoded.exp);

  const issuedBob = bob.clone();
  issuedBob.token = finite.data.token;
  const meFromFinite = await issuedBob.request('/api/auth/me');
  assert.equal(meFromFinite.data.user.id, bob.user.id);

  const never = await bob.request('/api/auth/tokens', {
    method: 'POST',
    json: { neverExpires: true },
  });
  assert.equal(never.data.expires_in_seconds, null);
  assert.equal(never.data.expires_at, null);
  assert.equal(never.data.never_expires, true);
  const neverDecoded = jwt.decode(never.data.token);
  assert.equal(neverDecoded.id, bob.user.id);
  assert.equal(Object.prototype.hasOwnProperty.call(neverDecoded, 'exp'), false);

  const invalid = await bob.request('/api/auth/tokens', {
    method: 'POST',
    json: { expiresInSeconds: 59 },
    expectedStatus: 400,
  });
  assert.equal(invalid.data.error, 'Invalid token lifetime');

  const adminIssued = await admin.request(`/api/admin/users/${bob.user.id}/tokens`, {
    method: 'POST',
    json: { expiresInSeconds: 60 },
  });
  const adminIssuedDecoded = jwt.decode(adminIssued.data.token);
  assert.equal(adminIssuedDecoded.id, bob.user.id);
  const adminIssuedSession = bob.clone();
  adminIssuedSession.token = adminIssued.data.token;
  const meFromAdminIssued = await adminIssuedSession.request('/api/auth/me');
  assert.equal(meFromAdminIssued.data.user.id, bob.user.id);

  const forbidden = await bob.request(`/api/admin/users/${admin.user.id}/tokens`, {
    method: 'POST',
    expectedStatus: 403,
  });
  assert.equal(forbidden.data.error, 'Admin only');
