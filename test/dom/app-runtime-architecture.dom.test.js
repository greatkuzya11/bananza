...
test('performance baseline helper is loaded early and initial app scripts stay bounded', () => {
  ...
  assert.ok(appScripts.includes('/js/app/markdown.js'), 'Markdown renderer must be in the app script graph');
  assert.ok(appScripts.length <= 100, `initial /js/app script count ${appScripts.length} should stay at or below the lazy-load target`);
  [
