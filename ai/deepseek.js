...
function timeoutError() {
  const error = new Error('DeepSeek API request timed out');
...
async function postJson(url, body, { apiKey, timeoutMs = 60000 }) {
  ...
  try {
    ...
    if (!res.ok) throw apiRequestError(errorText(payload, `DeepSeek API request failed with HTTP ${res.status}`), res.status);
    return payload || {};
  } catch (error) {
