...
function requestHeaders(apiKey, extras = {}) {
  ...
  if (!key) throw new Error('Qwen API key is not configured for AI bots');
  return {
...
async function testConnection(options) {
  const result = await createChatCompletion({
    ...options,
    user: options.user || 'Say briefly that the Qwen API connection works.',
